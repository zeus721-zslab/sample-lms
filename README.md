...
- 도메인 + DNS 설정 (SSL 발급용)
- 외부에서 이미 실행 중인 MariaDB · Redis · Elasticsearch 컨테이너 (`zslab_network` 네트워크 공유)
...
# 5. gateway Nginx 프록시 설정 추가
# scripts/gateway-nginx-lms.conf 참고 후 적용

# 6. 스택 기동 (이미지 빌드 + 컨테이너 실행 + 마이그레이션)
bash scripts/up.sh
...
- [ ] **실시간 통계 (ELK)** — 현재 DB 집계 → Elasticsearch 실시간 집계로 고도화
